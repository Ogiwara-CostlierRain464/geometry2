...
/** \brief convert Transform to Transform msg*/
  ...
  SiloBufferCore::SiloBufferCore(ros::Duration cache_time, uint64_t max_node_size)
    : cache_time_(cache_time)
    , using_dedicated_thread_(false)
    , max_node_size_(max_node_size)
  {
    frames_ = new tf2::TransformStorage*[max_node_size]();
    std::fill(frames_, frames_ + max_node_size, nullptr);

    frame_each_mutex_ = new VRWLock[max_node_size]();
...
// thread safe
  ...
  struct TransformAccum
  {
    ...
    CompactFrameID gather(TimeCacheInterfacePtr cache, ros::Time time, std::string* error_string, tf2::ReadStat *stat = nullptr)
    {
      st_rotation_.m_floats[0] = cache->rotation_.m_floats[0];
      st_rotation_.m_floats[1] = cache->rotation_.m_floats[1];
      st_rotation_.m_floats[2] = cache->rotation_.m_floats[2];
      st_rotation_.m_floats[3] = cache->rotation_.m_floats[3];

      st_translation_.m_floats[0] = cache->vec[0];
      st_translation_.m_floats[1] = cache->vec[1];
      st_translation_.m_floats[2] = cache->vec[2];
