...
//    }
  ...
  struct TransformAccum
  {
    ...
    {
      tf2::Vector3 tmp(st.vec[0], st.vec[1], st.vec[2]);
      if (source)
      {
        source_to_top_vec = quatRotate(st.rotation_, source_to_top_vec) + tmp;
        source_to_top_quat = st.rotation_ * source_to_top_quat;
      }
      else
      {
        target_to_top_vec = quatRotate(st.rotation_, target_to_top_vec) + tmp;
